    ...
    public class StockFlManager : IStockFlService
    {
        ...
        public async Task<List<StockFlDto>> GetStockFlListAsync()
        {
...

            {
                ...
                var sql = "DECLARE @MUTABAKAT AS INT = " + mutabakat + " " +
                    "DECLARE @ONCEKIAY AS INT = month( DATEADD( mm, DATEDIFF( mm,0,GETDATE() ) -1, 0 ) ) " +
                    "DECLARE @BUAY AS INT = month ( DATEADD( mm, DATEDIFF( mm,0,GETDATE() ), 0 ) )" +
                    "SELECT DataSourceCode = CASE STL.SOURCEINDEX WHEN 35 THEN 'AYKIZM' WHEN 7 THEN 'AYKANT' WHEN 42 THEN 'AYKKNY' WHEN 50 THEN 'AYKIST' END, " +
