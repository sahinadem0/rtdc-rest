    ...
    public class StockLvManager : IStockLvService
    {
        ...
        public async Task<List<StockLvDto>> GetStockLvListAsync()
        {
...

            {
                ...
                var sql = " DECLARE @MUTABAKAT INT = " + mutabakat + " "+
                    ...
                    "StockDate = CASE WHEN @MUTABAKAT = 1 THEN DATEADD(ss, -1, DATEADD(month, DATEDIFF(month, 0, getdate()), 0))  ELSE getdate() END, "+
                    ...
                    "AND StCardPort.SPECODE IN('3M','BPT','WL') AND StFichePort.DATE_ <= CASE WHEN @MUTABAKAT = 1 THEN DATEADD(ss, -1, DATEADD(month, DATEDIFF(month, 0, getdate()), 0)) ELSE StFichePort.DATE_ END "+
