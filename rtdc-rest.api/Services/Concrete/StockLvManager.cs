        ...
        public async Task<List<StockLvDto>> GetStockLvListAsync()
        {
            ...
            //Server = 172.16.40.20; Database = AYK2008; Persist Security Info = True; User ID = PG; Password = PG2007
            {
                ...
                var sql = " SELECT DataSourceCode = CASE StLinePort.SOURCEINDEX WHEN 35 THEN 'AYKIZM' " +
                    ...
                    ",ListPrice = (SELECT TOP (1) PRICE FROM LG_412_PRCLIST AS PRC " +
                    "WHERE (StCardPort.LOGICALREF = CARDREF) AND ( PTYPE = 1) " +
                    "ORDER BY BEGDATE DESC ) FROM LG_412_12_STFICHE StFichePort WITH(NOLOCK) " +
                    ...
                    "LEFT OUTER JOIN LG_XT1001_412 AS EK ON StCardPort.LOGICALREF = EK.PARLOGREFWHERE StLinePort.LINETYPE IN (0,1) " +
                ...
                var result = connection.Query<StockLvDto>(sql).ToList();
                return result;
            }
        }
    }
